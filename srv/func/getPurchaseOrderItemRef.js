...
module.exports = async (request, tx) => {
    ...
    if (!companyCode || !purchaseOrderRef) {
    ...
    const oResultPurchaseOrderItemRefRequest = await serviceRequestS4_HANA.get(process.env['Path_API_purchaseorder']+"/"+purchaseOrderRef+"/"+process.env['Path_API_purchaseorder_2']+"&$filter=CompanyCode eq '"+companyCode+"'");
    return { status: 200, result: oResultPurchaseOrderItemRefRequest, message: 'Executed' };
};